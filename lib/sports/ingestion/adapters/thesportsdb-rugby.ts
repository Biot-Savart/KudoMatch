...
const COMPETITION_META: Record<string, Pick<CanonicalCompetitionDTO, 'name' | 'kind' | 'country'>> = {
...
function readJsonEnv(name: string, fallback: Record<string, string>): Record<string, string> {
...
function editionParts(editionExternalKey: string): { leagueId: string; season: string } {
	const separator = editionExternalKey.indexOf('-');
	if (separator < 1) throw new Error(`Invalid TheSportsDB edition key '${editionExternalKey}'`);
	return {
...
export class TheSportsDbRugbyAdapter implements SportProviderAdapter {
	...
	constructor(options: TheSportsDbRugbyAdapterOptions = {}) {
		...
		this.baseUrl = (options.baseUrl || process.env.THESPORTSDB_BASE_URL || 'https://www.thesportsdb.com/api/v1/json').replace(/\/$/, '');
		this.leagues = options.leagues || readJsonEnv('THESPORTSDB_RUGBY_LEAGUES', DEFAULT_LEAGUES);
		this.seasons = options.seasons || readJsonEnv('THESPORTSDB_RUGBY_SEASONS', DEFAULT_SEASONS);
	}

	private async requestEvents(leagueId: string, season: string): Promise<TheSportsDbEvent[]> {
		...
		const payload = await withRetry(async () => {
			...
			if (!response.ok) throw new Error(`TheSportsDB HTTP ${response.status}: ${response.statusText}`);
			return await response.json() as { events?: TheSportsDbEvent[] | null };
	...
	async fetchCompetitions(): Promise<CanonicalCompetitionDTO[]> {
		return Object.entries(this.leagues).filter(([slug, id]) => Boolean(id && COMPETITION_META[slug])).map(([slug, externalKey]) => ({
			externalKey,
	...
	async fetchEditions(competitionExternalKey: string): Promise<CanonicalEditionDTO[]> {
		...
		const dates = events.map((event) => event.strTimestamp || event.dateEvent).filter(Boolean).sort();
		const startsAt = dates[0] ? new Date(dates[0]).toISOString() : undefined;
		const allFinished = events.length > 0 && events.every((event) => String(event.strStatus || '').toUpperCase() === 'FT');
		const status = allFinished ? 'completed' : startsAt && new Date(startsAt).getTime() > Date.now() ? 'planned' : 'active';
		const competitionSlug = Object.entries(this.leagues).find(([, id]) => id === competitionExternalKey)?.[0];
		const competitionName = competitionSlug ? COMPETITION_META[competitionSlug]?.name : undefined;
		return [{
			...
			startsAt,
			endsAt: dates.at(-1) ? new Date(dates.at(-1) as string).toISOString() : undefined,
			status,
			metadata: { source: 'TheSportsDB', freeTierEventLimit: 15 },
		}];
	}

	async fetchCompetitors(editionExternalKey: string): Promise<CanonicalCompetitorDTO[]> {
		...
		for (const event of events) {
			if (event.idHomeTeam && event.strHomeTeam) teams.set(event.idHomeTeam, { externalKey: event.idHomeTeam, name: event.strHomeTeam, shortName: event.strHomeTeam.replace(/ Rugby$/i, ''), mediaUrl: event.strHomeTeamBadge, kind: 'team', isActive: true });
			if (event.idAwayTeam && event.strAwayTeam) teams.set(event.idAwayTeam, { externalKey: event.idAwayTeam, name: event.strAwayTeam, shortName: event.strAwayTeam.replace(/ Rugby$/i, ''), mediaUrl: event.strAwayTeamBadge, kind: 'team', isActive: true });
		}
		if (teams.size === 0) throw new Error(`TheSportsDB returned no Rugby competitors for '${editionExternalKey}'`);
	...
	async fetchEvents(options: FetchEventsOptions): Promise<CanonicalEventDTO[]> {
		...
		return this.transformEvents(await this.requestEvents(leagueId, options.seasonKey || season), options.editionExternalKey);
	}
	...
	public transformEvents(events: TheSportsDbEvent[], editionExternalKey: string): CanonicalEventDTO[] {
		return events.filter((event) => event.idEvent && event.idHomeTeam && event.idAwayTeam && event.strTimestamp).map((event) => {
			const rawStatus = String(event.strStatus || '').toUpperCase();
			const status = event.strPostponed === 'yes' ? 'postponed' : normalizeEventStatus(rawStatus === 'FT' ? 'FT' : rawStatus || 'NS');
			const kickoffTime = new Date(event.strTimestamp as string).toISOString();
			...
			if (hasScores) winnerRole = homeScore! > awayScore! ? 'home' : awayScore! > homeScore! ? 'away' : 'draw';
			...
			return {
				...
				participants: [
					{ competitorExternalKey: String(event.idHomeTeam), role: 'home', slotNumber: 1 },
					{ competitorExternalKey: String(event.idAwayTeam), role: 'away', slotNumber: 2 },
				],
				market: {
				...
				result: hasScores ? { status: resultStatus, resultPayload: { homeScore, awayScore, winnerRole }, revisionNumber: 1 } : undefined,
				metadata: { provider: 'thesportsdb', league: event.strLeague, country: event.strCountry, sourceStatus: event.strStatus },
			};
		});
	}
}
